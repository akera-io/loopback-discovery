...
Discoverer.prototype.discoverModels = function(options, cb) {
  ...
  var casePropertiesDataType = function(model) {
  ...
  var saveModel = function(err, model) {
    if (err !== null)
      ...
      if (!utils.fileExists(modelPath) || options.overwrite === true) {
        console.log('Save definition for model: ' + model.name);
        casePropertiesDataType(model);
        models.push(model);
  ...
  options.schema = self.dataSource.settings.database;
